...
public class FluidMoveMethod extends AbstractModuleMethodExecutable<Object> {
    ...
    public FluidMoveMethod(String methodName, FluidManager fluidManager) {
        super("Moves fluid at the specified slot in the \"from\" fluid inventory to the \"to\" fluid inventory.", 
                "Number",
                "Amount of fluid moved.");
        this.fluidManager = fluidManager;
        this.methodName = methodName;

        addParameter("fluidInventoryBindingFrom", "FluidInventoryBinding", "Fluid inventory it should extract the " +
                "fluid from.");
        addParameter("fluidInventoryBindingTo", "FluidInventoryBinding", "Fluid inventory it should insert the fluid " +
                "to.");
    ...
    public Object onFunctionEnd(int line, ComputerCallback computer, Map<String, Variable> parameters,
        ...
        InventoryBinding.InventoryWithSlots inventoryFrom =
        ...
        InventoryBinding.InventoryWithSlots inventoryTo =
                FluidFunctionParamValidationUtil.validateFluidInventoryBinding(line, computer,
                parameters, "fluidInventoryBindingTo", methodName, true);

        int slotFrom = FunctionParamValidationUtil.validateSlotNo(line, parameters, inventoryFrom, "slotFrom",
                methodName);
        ...
        return fluidManager.moveFluid(computer.getComputerEntity(), inventoryFrom.inventory, inventoryTo.inventory,
                realSlotFrom, fluidType, realSlotTo, volume);
    }
}
