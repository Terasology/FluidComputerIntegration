...
public class FluidStorageModuleServerSystem extends BaseComponentSystem {
    ...
    public void computerModuleSlotChanged(InventorySlotChangedEvent event, EntityRef computerEntity,
        ...
        if (!computerIsMoving) {
            ...
            if (oldModule != null && oldModule.moduleType.equals(FluidStorageModuleCommonSystem.COMPUTER_FLUID_STORAGE_MODULE_TYPE)) {
                EntityRef inventoryEntity =
                        computerEntity.getComponent(FluidInternalStorageComponent.class).inventoryEntity;
        ...
        if (newModule != null && newModule.moduleType.equals(FluidStorageModuleCommonSystem.COMPUTER_FLUID_STORAGE_MODULE_TYPE)) {
            ...
            FluidInventoryComponent inventoryComponent =
                    new FluidInventoryComponent(computerModuleByType.getSlotCount(),
                            computerModuleByType.getMaxVolume());
    ...
    public void computerMovedCopyInternalStorage(BlockTransitionDuringMoveEvent event, EntityRef entity,
                                                 FluidInternalStorageComponent storage) {
        EntityRef inventoryEntity = storage.inventoryEntity;
        EntityRef newInventoryEntity =
                event.getIntoEntity().getComponent(FluidInternalStorageComponent.class).inventoryEntity;
